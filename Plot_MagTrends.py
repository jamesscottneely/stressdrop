...
#### Create plot type 1
####################
fig,ax = plt.subplots(nrows=6,ncols=2,figsize=(10,25),sharey=True)
### Plot the truth
ax[0,0].semilogy(merge_df.mw_tot,merge_df.sigma_tot_area,'o',alpha=.1)
...
### Plot the truth
ax[0,1].semilogy(merge_df.mw_tot,merge_df.sigma_tot_moment,'o',alpha=.1)
...
for idx in np.arange(0,5,1):
    ...
    #### n=?
    ax[idx+1,1].semilogy(merge_dfTEMP.mw_tot, merge_dfTEMP.SIG_n_Mo, 'o', alpha=.1)
    median_bin(merge_dfTEMP.mw_tot, merge_dfTEMP.SIG_n_Mo, ax[idx+1,1])
    ax[idx+1,1].set_xlabel('Mw')
    ax[idx+1,1].set_ylabel(r'$\Delta\sigma$ (Fc n=?)')
