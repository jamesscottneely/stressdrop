...
#### Smooth spectra
freq_smooth, amp_smooth = sf.resampSpec(freq_1,amplitude_1, log_samp)  ## Resample spectra
### Fit Fc only
initialGuess = [1]  ## Provide initial guess
outvalMoFixed = optimize.least_squares(sf.bruneModInvMoFixed, initialGuess, bounds=(.0001, np.inf),
                                       args=(amp_smooth, freq_smooth, mo_main_1))
est_fc_MoFixed = outvalMoFixed.x[0]
sigma_fc_est_Mofix = sf.stressCircfc(est_fc_MoFixed, mo_main_1, k, beta)
### Fit fc and Mo
initialGuess = [1, mo_main_1 * 1.05]  ## Provide initial guess start
outval = optimize.least_squares(sf.bruneModInvMoFree, initialGuess, bounds=((.0001, .01), (np.inf, np.inf)),
                                args=(amp_smooth, freq_smooth))  ## making the actual fit
...
label1 = "Mo:{:.1E}" + "\n" +"%: [" +"{:.1f},"*len(pulseMomentpercent1) +"] " + "\n" +r'$\Delta\sigma$: ['+ "{:.1f},"*len(pulseStressDrop1)+"]" + "\n" +"fc:["+ "{:.1f},"*len(fc_sub_1)+"]"
ax[0].plot(t,pulse_sum_1,color='blue',label=label1.format(mo_main_1,*pulseMomentpercent1,*pulseStressDrop1,*fc_sub_1))
ax[0].legend()
...
label2 =  "Mo:{:.1E}" + "\n" +"%: [" +"{:.1f},"*len(pulseMomentpercent2) +"] " + "\n" +r'$\Delta\sigma$: ['+ "{:.1f},"*len(pulseStressDrop2)+"]" + "\n" +"fc:["+ "{:.1f},"*len(fc_sub_2)+"]"
ax[1].plot(t,pulse_sum_2,color='green',label=label2.format(mo_main_2,*pulseMomentpercent2,*pulseStressDrop2,*fc_sub_2))
ax[1].legend()
...
label3 = "Mo:{:.1E}" + "\n" +"%: [" +"{:.1f},"*len(pulseMomentpercent3) +"] " + "\n" +r'$\Delta\sigma$: ['+ "{:.1f},"*len(pulseStressDrop3)+"]" + "\n" +"fc:["+ "{:.1f},"*len(fc_sub_3)+"]"
ax[2].plot(t,pulse_sum_2,color='orange',label=label3.format(mo_main_3,*pulseMomentpercent3,*pulseStressDrop3,*fc_sub_3))
ax[2].legend()
#### STF Freq domain
ax[3].loglog(freq_1,amplitude_1,color='blue',lw=3,label=r'MW: {:.1f} $\Delta\sigma$:{:.1f}'.format(totalMw_1,sigma_Mo_1))
ax[3].loglog(freq_1,sim_brune_small,color='red',lw=3,label=r'MW: {:.1f}, $\Delta\sigma$:{:.1f}'.format(simp_mw_small,simp_stress_small))
ax[3].loglog(freq_1,sim_brune_large,color='purple',lw=3,label=r'MW: {:.1f}, $\Delta\sigma$:{:.1f}'.format(simp_mw_large,simp_stress_large))
ax[3].axvline(est_fc_MoFixed,color='blue',ls=':')
ax[3].axvline(est_fc_MoFree,color='blue',ls='--')
ax[3].legend()

ax[4].loglog(freq_1,amplitude_1,color='blue',lw=3,label=r'MW: {:.1f} $\Delta\sigma$:{:.1f}'.format(totalMw_1,sigma_Mo_1))
ax[4].loglog(freq_2,amplitude_2,color='green',lw=3,label=r'MW: {:.1f} $\Delta\sigma$:{:.1f}'.format(totalMw_2,sigma_Mo_2))
ax[4].loglog(freq_1,sim_brune_large,color='purple',lw=3,label=r'MW: {:.1f} $\Delta\sigma$:{:.1f}'.format(simp_mw_large,simp_stress_large))
ax[4].axvline(est_fc_MoFixed,color='blue',ls=':')
ax[4].axvline(est_fc_MoFree,color='blue',ls='--')
ax[4].legend()

ax[5].loglog(freq_1,amplitude_1,color='blue',lw=3,label=r'MW: {:.1f} $\Delta\sigma$:{:.1f}'.format(totalMw_1,sigma_Mo_1))
ax[5].loglog(freq_2,amplitude_2,color='green',lw=3,label=r'MW: {:.1f} $\Delta\sigma$:{:.1f}'.format(totalMw_2,sigma_Mo_2))
ax[5].loglog(freq_1,amplitude_3,color='orange',lw=3,label=r'MW: {:.1f} $\Delta\sigma$:{:.1f}'.format(totalMw_3,sigma_Mo_3))
ax[5].axvline(est_fc_MoFixed,color='blue',ls=':')
ax[5].axvline(est_fc_MoFree,color='blue',ls='--')
ax[5].legend()


#### STF Ratios
ax[6].loglog(freq_1,amplitude_1/sim_brune_small,color='red',lw=3,label=r'MW: {:.1f}/MW: {:.1f}, $\Delta\sigma$ {:.1f} MAIN'.format(totalMw_1,simp_mw_small,simp_stress_small))
ax[6].loglog(freq_1,sim_brune_large/amplitude_1,color='purple',lw=3,label=r'MW: {:.1f}, $\Delta\sigma$ {:.1f}/MW: {:.1f} EGF'.format(simp_mw_large,simp_stress_large,totalMw_1))
...
ax[7].loglog(freq_1,sim_brune_large/amplitude_1,color='purple',lw=3,label=r'MW: {:.1f}, $\Delta\sigma$ {:.1f}/MW: {:.1f} EGF'.format(simp_mw_large,simp_stress_large,totalMw_1))
