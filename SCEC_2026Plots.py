...
def median_bin(xvals, yvals, ax):
    ...
    for m in m_pts:
        ...
        try:
            ...
            ax.errorbar(m_mo, median_bin, yerr=asymmetric_error, color='black', fmt='o', markeredgecolor='black', capsize=5,
                        markerfacecolor='yellow')
            ax.text(m_mo, median_bin + median_bin * 0.1, "{:.2f}".format(median_bin), va='bottom', ha='center',
...
merge_df['moment_i_float'] = merge_df['moment_i'].apply(lambda x: [float(i) for i in x[1:-1].split(',')])
merge_df['moment_top2']  = merge_df['moment_i_float'].apply(lambda x: sorted(x)[-2]/sorted(x)[-1])
merge_df['max_mo'] = [max(x) for x in merge_df['moment_i_float']]
merge_df['mo_ratio'] = merge_df['max_mo']/merge_df['moment_tot']
merge_df['stress_i_float'] = merge_df['stress_i'].apply(lambda x: [float(i) for i in x[1:-1].split(',')])
merge_df['stress_dif'] = merge_df['stress_i_float'].apply(lambda x: sorted(x)[0]/sorted(x)[-1])


HZ_low = HZ_Bands[2][0]
HZ_high = HZ_Bands[2][1]

cval= 'mo_ratio'
merge_df.sort_values(by=[cval],inplace=True)

fig,ax = plt.subplots(nrows=1,ncols=2,figsize=(10,4),sharey=True)
### Plot the truth
print(merge_df.iloc[0])
p1 = ax[0].scatter(merge_df[merge_df.MaxHz==HZ_high].sigma_tot_area,merge_df[merge_df.MaxHz==HZ_high].SIG_2_Mo,c=merge_df[merge_df.MaxHz==HZ_high][cval], cmap='viridis',alpha=.5)
med = np.median(merge_df[merge_df.MaxHz==HZ_high].SIG_2_Mo/merge_df[merge_df.MaxHz==HZ_high].sigma_tot_area)
sd_ln = np.std(merge_df[merge_df.MaxHz==HZ_high].SIG_2_Mo/merge_df[merge_df.MaxHz==HZ_high].sigma_tot_area)
corr = np.corrcoef(merge_df[merge_df.MaxHz==HZ_high].sigma_tot_area,merge_df[merge_df.MaxHz==HZ_high].SIG_2_Mo)[0,1]
ax[0].set_xscale('log')
ax[0].set_yscale('log')
textstr = "Med Ratio: {:.1f} SD Ratio: {:.1f} Corr: {:.2f}".format(med,sd_ln,corr)
ax[0].text(.99,.01,textstr,ha='right',va='bottom',transform=ax[0].transAxes)
ax[0].set_ylabel(r'$\Delta\sigma$ MPa (MoFix)')
ax[0].set_xlabel(r'$\Delta\sigma$ (True - Area) MPa')
ax[0].set_title('MinHZ: {:.3f} MaxHz: {:.3f}'.format(HZ_low,HZ_high))
axx = ax[0].get_xlim()
axy = ax[0].get_ylim()
...
p2 = ax[1].scatter(merge_df[merge_df.MaxHz==HZ_high].sigma_tot_moment,merge_df[merge_df.MaxHz==HZ_high].SIG_2_Mo,c=merge_df[merge_df.MaxHz==HZ_high][cval], cmap='viridis',alpha=.5)
med = np.median(merge_df[merge_df.MaxHz==HZ_high].SIG_2_Mo/merge_df[merge_df.MaxHz==HZ_high].sigma_tot_moment)
sd_ln = np.std(merge_df[merge_df.MaxHz==HZ_high].SIG_2_Mo/merge_df[merge_df.MaxHz==HZ_high].sigma_tot_moment)
corr = np.corrcoef(merge_df[merge_df.MaxHz==HZ_high].sigma_tot_moment,merge_df[merge_df.MaxHz==50].SIG_2_Mo)[0,1]
textstr = "Med Ratio: {:.1f} SD Ratio: {:.1f} Corr: {:.2f}".format(med,sd_ln,corr)
ax[1].text(.99,.01,textstr,ha='right',va='bottom',transform=ax[1].transAxes)
ax[1].set_ylabel(r'$\Delta\sigma$ MPa (MoFix)')
ax[1].set_xlabel(r'$\Delta\sigma$ (True - Moment) MPa')
ax[1].plot([axmin,axmax],[axmin,axmax],ls='--',lw=1)
ax[1].set_title('MinHZ: {:.3f} MaxHz: {:.3f}'.format(HZ_low,HZ_high))
...
### Plot the truth
print(merge_df.iloc[0])
p1 = ax[0].scatter(merge_df[merge_df.MaxHz==HZ_high].sigma_tot_area,merge_df[merge_df.MaxHz==HZ_high].SIG_2_MoFree,c=merge_df[merge_df.MaxHz==HZ_high][cval], cmap='viridis',alpha=.5)
med = np.median(merge_df[merge_df.MaxHz==HZ_high].SIG_2_MoFree/merge_df[merge_df.MaxHz==HZ_high].sigma_tot_area)
sd_ln = np.std(merge_df[merge_df.MaxHz==HZ_high].SIG_2_MoFree/merge_df[merge_df.MaxHz==HZ_high].sigma_tot_area)
corr = np.corrcoef(merge_df[merge_df.MaxHz==HZ_high].sigma_tot_area,merge_df[merge_df.MaxHz==HZ_high].SIG_2_MoFree)[0,1]
ax[0].set_xscale('log')
ax[0].set_yscale('log')
textstr = "Med Ratio: {:.1f} SD Ratio: {:.1f} Corr: {:.2f}".format(med,sd_ln,corr)
ax[0].text(.99,.01,textstr,ha='right',va='bottom',transform=ax[0].transAxes)
ax[0].set_ylabel(r'$\Delta\sigma$ MPa (MoFree)')
ax[0].set_xlabel(r'$\Delta\sigma$ (True - Area) MPa')
ax[0].set_title('MinHZ: {:.3f} MaxHz: {:.3f}'.format(HZ_low,HZ_high))
axx = ax[0].get_xlim()
axy = ax[0].get_ylim()
...
ax[0].plot([axmin,axmax],[axmin,axmax],ls='--',lw=1)
plt.colorbar(p1, label=cval)
p2 = ax[1].scatter(merge_df[merge_df.MaxHz==HZ_high].sigma_tot_moment,merge_df[merge_df.MaxHz==HZ_high].SIG_2_MoFree,c=merge_df[merge_df.MaxHz==HZ_high][cval], cmap='viridis',alpha=.5)
med = np.median(merge_df[merge_df.MaxHz==HZ_high].SIG_2_MoFree/merge_df[merge_df.MaxHz==HZ_high].sigma_tot_moment)
sd_ln = np.std(merge_df[merge_df.MaxHz==HZ_high].SIG_2_MoFree/merge_df[merge_df.MaxHz==HZ_high].sigma_tot_moment)
corr = np.corrcoef(merge_df[merge_df.MaxHz==HZ_high].sigma_tot_moment,merge_df[merge_df.MaxHz==HZ_high].SIG_2_MoFree)[0,1]
textstr = "Med Ratio: {:.1f} SD Ratio: {:.1f} Corr: {:.2f}".format(med,sd_ln,corr)
ax[1].text(.99,.01,textstr,ha='right',va='bottom',transform=ax[1].transAxes)
ax[1].set_ylabel(r'$\Delta\sigma$ MPa (MoFree)')
ax[1].set_xlabel(r'$\Delta\sigma$ (True - Moment) MPa')
ax[1].set_title('MinHZ: {:.3f} MaxHz: {:.3f}'.format(HZ_low,HZ_high))
ax[1].plot([axmin,axmax],[axmin,axmax],ls='--',lw=1)
plt.colorbar(p2, label=cval)
...
for idx in np.arange(0,3,1):
    ...
    p1 = ax[idx, 0].scatter(merge_dfTEMP.moment_tot,merge_dfTEMP.SIG_2_Mo ,c=merge_dfTEMP[cval], cmap='viridis',alpha=.5)
    plt.colorbar(p1, label=cval)
    bestfitLine_loglog(merge_dfTEMP.moment_tot,merge_dfTEMP.SIG_2_Mo , ax[idx, 0], 'orange')
    ax[idx,0].set_xscale('log')
    ax[idx, 0].set_yscale('log')
    ax[idx, 0].set_xlabel('Mo')
...
for idx in np.arange(0,3,1):
    ...
    #### PLot FC values
    p2 = ax[idx,1].scatter(merge_dfTEMP.Est_MoFree, merge_dfTEMP.Fc_2_MoFree, c=merge_dfTEMP[cval], cmap='viridis',alpha=.5)
    plt.colorbar(p2, label=cval)
    ax[idx,1].set_xlabel('Mo')
    ax[idx,1].set_ylabel(r'$f_c$ (Hz)')
    ax[idx, 1].set_xlim([5e15, 5e21])
    ...
    ax[idx, 2].set_title("Frequency Range [{:.1e},{:.1e}] Hz".format(HZ_Bands[idx][0], HZ_Bands[idx][1]))
    # ax[idx, 2].legend()
    median_bin(sf.mo2mw(merge_dfTEMP.Est_MoFree), merge_dfTEMP.SIG_2_MoFree/merge_dfTEMP.sigma_tot_moment,ax[idx, 2])
